...
> **Deviation found and closed, 2026-09-20.** For a time no cue files were bundled (`assets/audio/` was empty), so `TimerAudioService` played the OS alert via `SystemSound.play`, which met "no player per beep" only trivially and not "files are preloaded". It now preloads three bundled cues (`countdown`, `transition`, `complete` — synthesized by `scripts/build_timer_cues.py`, see `ASSETS-LICENSE.md`) into one `just_audio` player each, created once in `preload()` and reused; the session is activated with the ducking configuration only around playback. If a cue cannot load or play it falls back to the OS alert, and no audio failure reaches the timer. The `just_audio` seam is `TimerCuePlayer`, so the behaviour is unit-tested without a platform channel (`test/unit/timer_audio_service_test.dart`). **Not yet verified by ear on a physical device, nor on iOS at all** (audio-session ducking there is unchecked).
...
> **Amended 2026-09-24 — older database schemas are upgraded, not rejected.** Until now import compared `dbSchemaVersion` for equality, so any schema change made every existing export and account backup unrestorable — contrary to the "migrated by explicit, tested migration functions" rule above. `core/services/export_schema_upgrades.dart` now holds one step per schema version, each mirroring the `AppDatabase.onUpgrade` step of the same number, applied to exported rows. Exports from schema 5 (the oldest this repository has shipped) onward are upgraded; older and newer ones are refused with a clear message. **Every schema bump must add its step in the same change**; `test/unit/export_schema_upgrade_test.dart` fails CI otherwise, and restores real exports written by the schema 5, 7 and 9 builds (`test/fixtures/exports/`).
>
> **Also clarified — what "preserving the catalogue" keeps.** A populated local catalogue is still never overwritten or downgraded, but a replace now *adds* the backup's exercise rows this device lacks when they are custom or used by a backed-up workout or template. Custom exercises are user data that happen to live in the catalogue table; without this, restoring onto any fresh install (whose seeder has already filled the catalogue) failed on the first workout that used one. Their catalogue child rows (aliases, muscle links) are not restored. A missing exercise whose unique name or slug is already taken locally refuses the import before anything is written.
---
