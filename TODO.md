...
- [ ] **dev** Countdown timing quirk (found 2026-09-20 while verifying the cues; **predates this change**, not fixed). In a 20 s phase the countdown ticks fired at 16, 17 and 18 s and the phase changed at 20 s (logcat, two sessions) — two seconds of silence before the boundary instead of a 3-2-1 ending on it. Cause: `TimerController._maybeCountdown` keys on `snapshot.remaining.inSeconds`, which truncates, so the tick for "3" fires as soon as 3.99 s remain and nothing fires for the last second; `active_timer_page.dart` shows `UnitFormatters.duration(snapshot.remaining)`, which truncates the same way, so the digits read `00:00` for the final second. Fix both together — round *up* for the countdown (ticks and digits are 3-2-1 and `00:00` appears only at the boundary) — but change the timer page's call, not `UnitFormatters.duration` itself, which also formats elapsed session durations where truncation is correct. Add the countdown test that does not exist today (`timer_controller_test.dart` has none).
